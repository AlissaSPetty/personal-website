...
gulp.task('scripts-prod', function() {
 gulp.src(paths.js+'script.js', {cwd: bases.dev})
 .pipe(jshint())
 .pipe(jshint.reporter('default'))
 .pipe(uglify())
 .pipe(concat('script.min.js'))
 .pipe(gulp.dest(bases.prod + paths.js));
 gulp.src(paths.js+'vendor/*.js', {cwd: bases.dev})
...
gulp.task('imagemin-prod', function() {
 gulp.src(paths.images + '/**/*.*', {cwd: bases.dev})
 .pipe(imagemin())
...
gulp.task('watch', function() {
  ...
  livereload.listen();
  gulp.watch(bases.dev).on('change', livereload.changed);
});
